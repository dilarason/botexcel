...
def ask_field_type(key):
    """Kullanıcıdan yeni bir başlık için tip ve açıklama ister."""
...
def update_field_dict_for_keys(keys, field_dict):
    """
    Liste halindeki Key başlıklarında sözlükte olmayanları kullanıcıya sorar, ekler.
    keys: ["Adı", "BARCODE", ...]
    """
    changed = False
    for key in keys:
        if key not in field_dict:
            info = ask_field_type(key)
            field_dict[key] = info
            changed = True
    if changed:
